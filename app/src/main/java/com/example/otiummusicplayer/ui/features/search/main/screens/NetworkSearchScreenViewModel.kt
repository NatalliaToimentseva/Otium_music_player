...
import com.example.otiummusicplayer.ui.features.search.main.LoadAlbumByArtistUseCase
...
class NetworkSearchScreenViewModel @Inject constructor(
    ...
    private fun searchTracksByQuery() {
        if (state.value.searchValue.isBlank()) {
            ...
            viewModelScope.launch(Dispatchers.IO) {
//                searchTracksUseCase.searchTrackByQuery(state.value.searchValue).collect { result ->
//                    handleResult(result)
//                }
                state.tryEmit(
                    state.value.copy(
...
//            is NetworkResult.SuccessSearchTracks -> state.tryEmit(
//                state.value.copy(
//                    isLoading = false,
